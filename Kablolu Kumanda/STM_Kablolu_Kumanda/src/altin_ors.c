...
int main(void)
{
    ...
    while (1)
    {
        ...
        if (val != -1)
        {
            ...
            {
            case 0:        // Söküm Yukarı
                bit10 = 1; // Söküm yukarı için bit10'u 1 yap
                break;
            case 8:       // Söküm Aşağı
                bit9 = 1; // Söküm aşağı için bit9'u 1 yap
                break;
            case 2:       // OK Sağ
                bit2 = 1; // OK sağ için bit2'yi 1 yap
                break;
            case 11:      // OK Sol
                bit1 = 1; // OK sol için bit1'i 1 yap
                break;

            case 15:      // Depo Yukarı
                bit3 = 1; // Depo yukarı için bit3'ü 1 yap
                break;
            case 1: // Makina Açık
                if (!makina_btn_flag)
                {
                ...
                break;
            case 7:       // Depo Aşağı
                bit4 = 1; // Depo aşağı için bit4'ü 1 yap
                break;
            case 12:       // Hazırlayıcı Yukarı
                bit11 = 1; // Hazırlayıcı yukarı için bit11'i 1 yap
                break;
            case 13:       // Hazırlayıcı Aşağı
                bit12 = 1; // Hazırlayıcı aşağı için bit12'yi 1 yap
                break;
            case 9:       // İlave Aç
                bit7 = 1; // İlave aç için bit7'yi 1 yap
                break;
            case 10:      // İlave Kapat
                bit8 = 1; // İlave kapat için bit8'i 1 yap
                break;
            ...
            case 5: // Boşaltma Durdur
                ...
                bit6 = 1;
                break;
            case 6:        // Yedek Ok Yukarı
                bit15 = 1; // Yedek ok yukarı için bit15'i 1 yap
                break;
            case 14:       // Yedek Ok Aşağı
                bit14 = 1; // Yedek ok aşağı için bit14'ü 1 yap
                break;
            case 3: // Otomatik On-Off
                if (!otomatik_btn_flag)
                {
        ...
        else
        {
            ...
            otomatik_btn_flag = 0;   // Depo buton bırakıldıysa flag sıfırlanır
            depo_btn_flag = 0;       // Depo buton bırakıldıysa flag sıfırlanır
            GPIOA->ODR &= ~(1 << 1); // LED söndür

            if (otomatik_aktif)
            {
                bit0 = 1;               // Otomatik aktifse bit0'ı 1 yap
                GPIOA->ODR |= (1 << 2); // PA2 pinini high yapar
            }
            else
            {
                bit0 = 0;                // Otomatik pasifse bit0'ı 0 yap
            ...
            bit1 = 0, bit2 = 0, bit3 = 0, bit4 = 0, bit6 = 0, bit7 = 0, bit8 = 0, bit9 = 0, bit10 = 0, bit11 = 0, bit12 = 0, bit14 = 0, bit15 = 0;
        }
